...
<html>
    <head>
        ...
        <title>Category: Discussions</title>
    ...
    <body>
        ...
        <div class="container" style="margin-top: 30px;">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="g-description">
            </div>
            <button type="button" class="btn btn-default create-group">Lưu</button>
        </div>
        <script type=text/javascript>
...
//                            var socket = io('192.168.1.4:8000');
        ...
        <script type="text/javascript">
            $(".create-group").click(function () {
                if ($('#g-name').val() == '' || $('#g-description').val() == '') {
                    alert("Chưa nhập đủ thông tin");
                } else {
                    $.ajax({
                        url: "<%=request.getContextPath()%>/groups/create/save",
                        type: "post",
                        dataType: "text",
                        data: {
                            g_name: $('#g-name').val(),
                            g_description: $('#g-description').val()
                        },
                        success: function (data) {
                            var resultJson = JSON.parse(data);
                            if (resultJson[0].type == "ACCESS") {
                                $(location).attr('href', '<%= request.getContextPath()%>/groups/' + resultJson[1].depscription);
                            } else {
