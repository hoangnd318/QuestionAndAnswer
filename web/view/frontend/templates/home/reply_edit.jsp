    ...
    <body>
            ...
            <div class="col-full push-top">
                ...
                <form action="">
                    <div class="form-group">
                        ...
                        <textarea id="thread_content" class="form-input" name="content" rows="8" cols="140">${reply.content}</textarea>
                    </div>

                    <div class="btn-group">
                        <button class="btn btn-blue edit-reply" type="button" name="Lưu">Lưu </button>
                    </div>
                </form>
            </div>
        ...
        <script type="text/javascript">
            $(".edit-reply").click(function () {
                if (CKEDITOR.instances['thread_content'].getData() == '') {
                    alert("Chưa nhập đủ thông tin");
                } else {
                    $.ajax({
                        url: "<%=request.getContextPath()%>/reply/edit/save",
                        type: "post",
                        dataType: "text",
                        data: {
                            r_content: CKEDITOR.instances['thread_content'].getData(),
                            r_id: '${reply.id}'
                        },
                        success: function (data) {
                            var resultJson = JSON.parse(data);
                            if (resultJson[0].type == "ACCESS") {
                                var url_tmp = '<%= request.getParameter("url")%>';
                                $(location).attr('href', '<%= request.getContextPath()%>/threads' + url_tmp);
                            } else {
